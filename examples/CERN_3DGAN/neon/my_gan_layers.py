...
def discriminator():
    ...
    if mgc_my_xavier_discr:
        init = Xavier()
    else:
        init = Gaussian(scale=mgc_gaussian_scale_init_for_discriminator)
    ...
    if mgc_discriminator_option == 1:
        ...
        branch0 = [b1,
                    ...
                    Dropout(keep=mgc_drop_out_rate),
    ...
    elif mgc_discriminator_option == 2:
        conv1 = dict(init=init, batch_norm=False, activation=lrelu, bias=init)
        conv2 = dict(init=init, batch_norm=False, activation=lrelu, padding=2, bias=init)
        conv3 = dict(init=init, batch_norm=False, activation=lrelu, padding=1, bias=init)
        b1 = BranchNode("b1")
    ...
    D_layers = Tree([branch0, branch1, branch2], alphas=mgc_alpha) # Real/Fake, SUM ECAL, E primary
    return D_layers
...
def generator():
    ...
    if mgc_my_xavier_gen:
        init_gen = Xavier()
        init_gen_top = Xavier()
    else:
        init_gen = Gaussian(scale=mgc_gaussian_scale_init_for_generator)
        init_gen_top = Gaussian(scale=(mgc_gaussian_scale_init_for_generator_top_layer))

    if mgc_gen_top == "tanh":
        gen_top = Tanh()
    elif mgc_gen_top == "logistic":
        gen_top = Logistic(shortcut=False)
    elif mgc_gen_top == "lrelu":
        gen_top = lrelu
    else:
        gen_top = Rectlin()

    if mgc_generator_option == 1:
